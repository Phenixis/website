...
export default async function Page() {
    ...
    return (
        <section className="page space-y-4">
            ...
            <div>
                <h2 className="page-title text-2xl mb-3">
                    <Link href="/projects" underlined={false}>
                        My current projects
                    </Link>
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                    {projects.filter((project) => project.metadata.tags?.includes('Running') || project.metadata.tags?.includes('Building')).map((project) => (
