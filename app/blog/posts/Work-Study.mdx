...
summary: Implementation of E2E testings using Playwright.
color: green
tags:
  - Project
  - Running
...
1. You have a first level with:<br/>`test.describe("Page 1 test suite", async (): Promise<void> { ... })`
2. Then, there is another level with:<br/>`test("Verify if the title contains 'Mon titre'", async ({ page }): Promise<void> { ... })`
3. Finally, there is a third level:<br/>`await assert(page.locator("h1#title").textContent).contains("Mon titre");`
