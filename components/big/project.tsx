...
export default function Project({
    ...
    return (
        <Link href={`/blog/${name.replace(/ /g, '_')}`}>
            <div className={`size-full p-2 rounded-md duration-1000 flex flex-col ${showBadge ? "md:items-center" : ""} md:flex-row space-x-0 md:space-x-2 selection:${colorVariants[color].image} ${colorVariants[color].background} ${colorVariants[color].border}`}>
                {
                    showBadge ? (
