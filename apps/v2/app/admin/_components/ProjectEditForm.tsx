...
export function ProjectEditForm() {
  ...
  const isNew = id === "new";
  const basePath = pathname.startsWith("/admin/side-quests") ? "/admin/side-quests" : "/admin/projects";
  const panelLabel = basePath === "/admin/side-quests" ? "Side Quests" : "Main Projects";
  const newCategory = basePath === "/admin/side-quests" ? "side" : "main";

  const [p, setP] = useState<Project>(() => (isNew ? { ...BLANK, category: newCategory } : BLANK));
  ...
  useEffect(() => {
    ...
    fetch(`/api/projects/${id}`)
      ...
      .then((data) => {
        if (!data) { router.replace(basePath); return; }
        setP(data);
        setLoading(false);
      });
  ...
  const save = async () => {
    ...
    setSaving(false);
    setSavedAt(new Date());
    if (isNew) router.push(payload.category === "side" ? "/admin/side-quests" : "/admin/projects");
  };
      ...
      <div className={editPageCls.form}>
        ...
        <div className={editPageCls.formSide}>
          <div className={editPageCls.sideCard}>
            ...
            <div className={editPageCls.sideCardRow}>
              <span className={editPageCls.sideCardKey}>Panel</span>
              <span className={editPageCls.sideCardVal}>{p.category === "side" ? "Side Quests" : "Main Projects"}</span>
            </div>
            <div className={editPageCls.sideCardRow}>
          ...
          <FormSection title="Panel">
            <Field label="Panel" hint="Which public panel this shows up under">
              <Segmented
                value={p.category === "side" ? "side" : "main"}
                onChange={(v) => update("category", v)}
                options={[
                  { value: "main", label: "Main Projects" },
